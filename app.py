...
# JSON User Storage (Legacy Frontend User Management)
USERS_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'instance', 'user.json')
...
def forgot_password():
    if request.method == 'POST':
        ...
        if user_index != -1:
            ...
            flash(
                f'Reset link generated successfully! <br><br><strong>Link:</strong> <a href="{reset_link}" class="alert-link">{reset_link}</a>',
                'success')
        else:
...
def admin_user_edit(id):
    ...
    if request.method == 'POST':
        ...
        existing_user = User.query.filter(
            (User.id != id) & ((User.email == email) | (User.username == username))
        ).first()
...
def user_delete(id):
    ...
    if request.method == 'POST':
        try:
            if user.profile:
                image_path = os.path.join(app.root_path, 'static', user.profile)
